...
public class RepeatNode : RgxNode
{
    ...
    public RepeatNode(IList<IRgxNode> parameters, int least, int most = -1, bool isLazy = false) : base(
        RgxNodeType.Repeat, parameters.ToArray())
    {
        if (parameters.Count != 1) throw new ArgumentException("RepeatNode must have exactly one parameter node.");
        Least = least;
        Most = most;
        IsLazy = isLazy;
    }
    ...
    public override string DisplayName
    {
        get
        {
            var rep = (Least, Most) switch
            {
                ...
                var (least, most) when least == most => $"Rep({least})",
                var (least, most) => $"Rep({least}, {most})"
            };
            return rep;
        }
    }
    ...
    internal override string CalculateResult()
    {
        ...
        return (Parameters[0] == null ? "repeat Value" : Parameters[0]?.ProduceResult() ?? string.Empty).Rep(Least,
            Most, IsLazy);
    }
    ...
    public override string RandomMatch()
    {
        ...
        return string.Concat(
            Enumerable.Repeat(0, repeatCount).Select(i => Parameters[0]?.RandomMatch() ?? string.Empty));
    }
    ...
    public override string RawCode(CodeCollector cc)
    {
        ...
        var repCode = (Least, Most) switch
        {
            ...
            var (least, most) when least == most => $"{input}.Rep({least}, {most}{lazyArg})",
            var (least, most) => $"{input}.Rep({least}, {most}{lazyArg})"
        };
    ...
    protected override void RestoreSerializationData(Dictionary<string, JsonElement> data)
    {
        ...
        if (data.TryGetValue("Least", out var leastElement)) Least = leastElement.GetInt32();
        if (data.TryGetValue("Most", out var mostElement)) Most = mostElement.GetInt32();
        if (data.TryGetValue("IsLazy", out var isLazyElement)) IsLazy = isLazyElement.GetBoolean();
    }
}